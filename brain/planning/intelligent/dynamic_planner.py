...
class DynamicPlanner:
    ...
    def check_and_insert_preconditions(
        ...
        for precondition in node.preconditions:
            inserted = self._handle_precondition(precondition, node, task_name)
            if inserted:
    ...
    def _extract_door_name(self, precondition: str) -> Optional[str]:
        """从前置条件中提取门名称"""
        # 简单实现：查找door.state(door_name)或类似模式
        import re
        match = re.search(r'door\.state\((\w+)\)|door_(\w+)', precondition)
        if match:
    ...
    def _extract_object_name(self, precondition: str) -> Optional[str]:
        """从前置条件中提取物体名称"""
        import re
        match = re.search(r'object\.visible\((\w+)\)|object_location\((\w+)\)', precondition)
        if match:
            return match.group(1) or match.group(2)
        return None
